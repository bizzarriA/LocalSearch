...
int almenoUnNodo(arco ArcoAttuale,int* NodiAttuali){    //ritorna 1 se almeno uno dei due nodi in cui incide ArcoAttuale è già stato preso, ovvero se ArcoAttuale è adiacente ad un arco che fa già parte dell'albero
...
void greedyCostruttiva(arco* SoluzioneAttuale, int* NodiAttuali, int Numero_nodi,int* IdArchi, int Numero_archi,int Grado_massimo){
    ...
    while(!allNodes(NodiAttuali,Numero_nodi)) {
        ...
        i = rand() % (Numero_archi);
        IdArco = IdArchi[i];
        if (notInLista(IdArco, DaRicontrollare, j) && SoluzioneAttuale[IdArco - 1].Selected != 1) {
            if (is_percorso(SoluzioneAttuale[IdArco - 1], NodiAttuali) || gradoEccessivo(SoluzioneAttuale[IdArco - 1], NodiAttuali, Grado_massimo)) {
...
void greedyCostruttivaTabu(arco* SoluzioneAttuale, int* NodiAttuali, int Numero_nodi,int* IdArchi, int Numero_archi,int Grado_massimo){
    int i,IdArco;
    int j=0;
    int PrimoGiro=1;    //al posto della funzione che ogni volta controlla i gradi dei nodi per vedere se almeno uno è stato preso, setto una semplice variabile
    int DaRicontrollare[NUMEROARCHI]={0}; //contiene gli Id di archi che non potevano essere presi al momento perchè non adiacenti ad altri
    while(!allNodes(NodiAttuali,Numero_nodi)) {
        ...
        IdArco = IdArchi[i];
        if (notInLista(IdArco, DaRicontrollare, j)&&SoluzioneAttuale[IdArco - 1].Selected != 1) {
            if (is_percorso(SoluzioneAttuale[IdArco - 1], NodiAttuali)) {
                SoluzioneAttuale[IdArco - 1].Ammissibile = 0;
            } else if (PrimoGiro || almenoUnNodo(SoluzioneAttuale[IdArco - 1], NodiAttuali)) {
                PrimoGiro = 0;
                SoluzioneAttuale[IdArco - 1].Selected = 1;
                NodiAttuali[SoluzioneAttuale[IdArco - 1].N1 - 1]++;
                NodiAttuali[SoluzioneAttuale[IdArco - 1].N2 - 1]++;
                memset(DaRicontrollare, 0, (j + 1) *
                                           sizeof(int));   //svuota la lista dei nodi che non potevano essere presi perchè non adiacenti
                j = 0;
            } else {
                DaRicontrollare[j] = SoluzioneAttuale[IdArco - 1].Id;
                j++;
...
void main() {
    ...
    if(c=='G'){
        fp=fopen("C:\\Users\\Sara\\Documents\\GitHub\\LocalSearch\\DEGREE CONSTRAINED MST\\CREA ISTANZE\\nuova_istanza_greedy.pl","w");
        if(fp==NULL){
        ...
        fd=fopen("C:\\Users\\Sara\\Documents\\GitHub\\LocalSearch\\DEGREE CONSTRAINED MST\\CREA ISTANZE\\nuova_istanza_ls.txt", "r");
        if(fd==NULL){
            printf("Errore apertura file");
            exit(1);
        }
        ...
        fclose(fd);
        individuaId(ListaArchi,ListaId);
        greedyCostruttivaTabu(ListaArchi,ListaNodi,n,ListaId,count-1,g);
        fp=fopen("C:\\Users\\Sara\\Documents\\GitHub\\LocalSearch\\DEGREE CONSTRAINED MST\\CREA ISTANZE\\nuova_istanza_tabu.txt","w");
        if(fp==NULL){
            printf("Errore apertura file");
            exit(1);
