...
void modificaSoluzione(arco* ListaArchi, int* Nodi,int* Counter) {
    ...
    for (int i = 0; i < NUMERONODI - 1 && k<=NArchiRemove; i++) {
        trovato=0;
        // nel ciclo seleziona un arco di counter,lo togli, da uno dei suoi nodi,
        // prendi la rosa di archi e prendi un arco non selezionato che non fosse in counter.
        // Se crea un ciclo, toglilo e riprova, se non crea un ciclo sei a posto
        if (Counter[i] != -1) {
...
void main() {
    ...
    int Stallo = 0, NoImprovement=0, Repeat=0, poche=0;//poche serve per sistemare le stampe a seconda che l'istanza sia piccola o grande
    ...
    fd = fopen("C:\\Users\\alice\\OneDrive\\Documents\\GitHub\\LocalSearch\\CreaIstanze\\50nodi.txt", "r");
    ...
    while (!Stallo) {   //finchè non raggiungo lo stallo, continuo a cercare una soluzione migliore
        ...
        if (!Stallo) {
            ...
            if(trovato==1) {
                ...
                trovato=0;
                intersezioneCounter(Counter,IdRimosso); //mette -1 al posto dell'id modificato (se c'è in lista)
            }
