...
#define TABUSIZE 4              //dimensione massima lista Tabu
...
//idea generele, aggiungo un arco casuale e ne rimuovo un altro nel ciclo che si viene a creare, così per N volte.
...
void main() {
    ...
    int trovato=0, TabuOn=0, cicloTabu=0;
    ...
    while (!Stallo) {   //finchè non raggiungo lo stallo, continuo a cercare una soluzione migliore
        ...
        if(TabuOn==1 && CostoMiglioreAttuale<CostoMiglioreAssoluto){
